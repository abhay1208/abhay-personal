...
/**
 * Draws a sample assuming Gaussian distribution
 * @param (mean) - mean of the distribution
 * @param (std_dev) - standard deviation of the distribution
 * @output Sample from the distribution
 */

inline double getGaussianSample(double mean, double std_dev)
{
...
inline int generateRandomNum(int N)
{
  unsigned seed = std::chrono::system_clock::now().time_since_epoch().count();
  static std::default_random_engine generator(seed);
...
inline void printVector(std::vector<T1> v)
{
